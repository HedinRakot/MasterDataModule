...
{
    /// <summary>
    ///     Custom Controller for <see cref="SchoolInfo"/> entity
    /// </summary>
    public partial class SchoolInfosController
    {
        protected override string BuildWhereClause<T>(Filter filter)
        {
            if (filter.Field == "name")
            {
                ...
                clauses.AddRange(new[] { 
        				base.BuildWhereClause<T>(new Filter { Field = "Text", Operator = filter.Operator, Value = filter.Value }),
