...
{
    /// <summary>
    ///     Custom Controller for <see cref="OrgInspectionDevice"/> entity
    /// </summary>
    public partial class OrgInspectionDevicesController
    {
        protected override string BuildWhereClause<T>(Filter filter)
        {
            if (filter.Field == "name")
            {
                ...
                clauses.AddRange(new[] { 
        				base.BuildWhereClause<T>(new Filter { Field = "Identification", Operator = filter.Operator, Value = filter.Value }),
