...
namespace MetadataLoader.Excel.OfficeOpenXML
{
    ...
    public class OfficeProperties
    {
        ...
        {
            get { return (_uriPropertiesExtended); }
        }
        /// <summary>
        ...
        {
            get { return (_uriPropertiesCustom); }
        }
        #endregion
            ...
            get
            {
                if (_xmlPropertiesCore == null)
                {
                    ...
                    else
                    {
                        ...
                        var partCore = _xlPackage.Package.CreatePart(CorePropertiesUri,
                                                                             @"application/vnd.openxmlformats-package.core-properties+xml");

                        // create the document properties XML (with no entries in it)
                        _xmlPropertiesCore = new XmlDocument();
                        var root = _xmlPropertiesCore.CreateElement("cp:coreProperties", SchemaCore);
                        ...
                        var streamCore = new StreamWriter(partCore.GetStream(FileMode.Create, FileAccess.Write));
                        _xmlPropertiesCore.Save(streamCore);
                        streamCore.Close();
                        _xlPackage.Package.Flush();

                        // create the relationship between the workbook and the new shared strings part
                        _xlPackage.Package.CreateRelationship(CorePropertiesUri,
                                                              TargetMode.Internal,
        ...
        private string GetCorePropertyValue(string nameSpace, string propertyName)
        {
            ...
            var searchString = string.Format("//cp:coreProperties/{0}:{1}", nameSpace, propertyName);
            var node = CorePropertiesXml.SelectSingleNode(searchString, _nsManager);
            if (node != null)
            {
        ...
        private void SetCorePropertyValue(string nameSpace, string propertyName, string propValue)
        {
            ...
            if (node == null)
            {
                ...
                node = CorePropertiesXml.CreateElement(nameSpace, propertyName, schema);
            ...
            get
            {
                if (_xmlPropertiesExtended == null)
                {
                    ...
                    else
                    {
                        ...
                        var partExtended = _xlPackage.Package.CreatePart(ExtendedPropertiesUri,
                                                                                 @"application/vnd.openxmlformats-officedocument.extended-properties+xml");

                        // create the extended properties XML (with no entries in it)
                        _xmlPropertiesExtended = new XmlDocument();
                        var root = _xmlPropertiesExtended.CreateElement("Properties", SchemeExtended);
                        ExcelPackage.AddSchemaAttribute(root, SchemaVt, "vt");
                        _xmlPropertiesExtended.AppendChild(root);

                        // save it to the package
                        var streamExtended = new StreamWriter(partExtended.GetStream(FileMode.Create, FileAccess.Write));
                        _xmlPropertiesExtended.Save(streamExtended);
                        streamExtended.Close();
                        _xlPackage.Package.Flush();

                        // create the relationship between the workbook and the new shared strings part
                        _xlPackage.Package.CreateRelationship(ExtendedPropertiesUri,
                                                              TargetMode.Internal,
        ...
        private void SetExtendedPropertyValue(string propertyName, string propValue)
        {
            ...
            {
                // the property does not exist, so create the XML node
                node = ExtendedPropertiesXml.CreateElement(propertyName, SchemeExtended);
        ...
        public XmlDocument CustomPropertiesXml
        {
            get
            {
                if (_xmlPropertiesCustom == null)
                {
                    ...
                    {
                        // create a new extended properties part and add to the package
                        var partCustom = _xlPackage.Package.CreatePart(CustomPropertiesUri,
                                                                               @"application/vnd.openxmlformats-officedocument.custom-properties+xml");

                        // create the extended properties XML (with no entries in it)
                        _xmlPropertiesCustom = new XmlDocument();
                        var root = _xmlPropertiesCustom.CreateElement("Properties", SchemaCustom);
                        ExcelPackage.AddSchemaAttribute(root, SchemaVt, "vt");
                        _xmlPropertiesCustom.AppendChild(root);

                        // save it to the package
                        var streamCustom = new StreamWriter(partCustom.GetStream(FileMode.Create, FileAccess.Write));
                        _xmlPropertiesCustom.Save(streamCustom);
                        streamCustom.Close();
                        _xlPackage.Package.Flush();

                        // create the relationship between the workbook and the new shared strings part
                        _xlPackage.Package.CreateRelationship(CustomPropertiesUri,
                                                              TargetMode.Internal,
        ...
        public string GetCustomPropertyValue(string propertyName)
        {
            ...
            var node = CustomPropertiesXml.SelectSingleNode(searchString, _nsManager);
            if (node != null)
            {
        ...
        public void SetCustomPropertyValue(string propertyName, string propValue)
        {
            ...
            var searchString = @"//ctp:Properties";
            var allProps = CustomPropertiesXml.SelectSingleNode(searchString, _nsManager);
            ...
            if (node == null)
            {
                ...
                {
                    var attr = (XmlAttribute) prop.Attributes.GetNamedItem("pid");
                    if (attr != null)
                    {
                ...
                pid++;
                // the property does not exist, so create the XML node
                var element = CustomPropertiesXml.CreateElement("property", SchemaCustom);
                ...
                var valueElement = CustomPropertiesXml.CreateElement("vt", "lpwstr", SchemaVt);
