...
namespace MetadataLoader.Excel.OfficeOpenXML
{
    ...
    {
        #region Static
        public static ExcelPackage CreateNew(string filePath)
        {
            return new ExcelPackage(new FileInfo(filePath));
        }

        public static ExcelPackage CreateNew(string filePath, string templatePath)
        {
            #region Check
            if (string.Equals(filePath, templatePath))
            {
                throw new ArgumentException("New file path and template file path are same");
            }
            #endregion
            return new ExcelPackage(new FileInfo(filePath), new FileInfo(templatePath));
        }

        public static ExcelPackage Open(string filePath)
        {
            var info = new FileInfo(filePath);
            return new ExcelPackage(info, info);
        }
        #endregion
        #region Constants
        /// <summary>
        ...
        /// </summary>
        protected internal const string schemaRelationships =
        ...
        private ExcelPackage(FileInfo newFile, FileInfo template)
        {
            newFile.Refresh();
            template.Refresh();
            ...
            if (template.Exists)
            {
                if (template.FullName != newFile.FullName)
                {
                    if (newFile.Exists)
                    {
                        try
        ...
        protected internal void WriteDebugFile(XmlDocument XmlDoc, string subFolder, string FileName)
        {
            if (DebugMode)
            {
                var dir = new DirectoryInfo(_outputFolderPath + "/" + subFolder);
                if (!dir.Exists)
                {
                    dir.Create();
                }

                var file = new FileInfo(_outputFolderPath + "/" + subFolder + "/" + FileName);
                if (file.Exists)
                {
        ...
        {
            Uri uriShared = null;
            var partParent = _package.GetPart(uriParent);
            //  Get the Uri to the shared part
            foreach (
                var relationship in
                    partParent.GetRelationshipsByType(schemaRelationships + "/" + Relationship))
            {
        ...
        {
            var partPack = _package.GetPart(uriPart);
